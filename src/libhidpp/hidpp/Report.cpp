...
static bool hasReport(const HID::ReportCollection &collection, HID::ReportID::Type type, uint8_t id, uint32_t usage)
{
	using namespace HID;
	...
	if (auto usages = std::get_if<std::vector<uint32_t>> (&field.usages)) {
		return field.flags.isArray () && usages->size () == 1 && usages->front () == usage;
	}
	else
		return false;
}
